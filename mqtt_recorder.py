...
def mqtt_play(mqtt_file: str, mqtt_client: mqtt):
    global msg_count
    global should_quit
    ...
    while bitstream.pos < bitstream.length and not should_quit:
        ...
        while (time.time() - START_TIME < timestamp) and not should_quit: pass
...
def message_callback(client: mqtt, userdata: str, message: mqtt.MQTTMessage):
    global msg_count
...
def arg_parser(arguments_passed: bool) -> argparse:
    ...
    parser = argparse.ArgumentParser(description='MQTT message recorder and player', add_help=False)
...
def main(args: argparse) -> int:
    ...
    # # MQTT client
    mqtt_client = mqtt.Client(f'MQTT-recorder', userdata=mqtt_file)
    ...
    if args.rec:
        # Reserve first 8 bytes of the file
    ...
    if args.rec:
        with open(mqtt_file, 'r+b') as fp:
            fp.write(struct.pack('<Q', msg_count))
        
        print(f"Logged {msg_count} messages")
